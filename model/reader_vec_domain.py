...
class Reader(threading.Thread):
    ...
    def __init__(self, x_train, y_train,y_train_domain,ratio_subset = 0.92, rng_seed=123, n_nearby=128, batch_size=32, center_pad=True, valid_fold=None):
        """
...
#                 final_dim_time = int(self.dim_time*self.ratio_subset)
                ...
                label_domain = np.zeros((len(batch_index), self.dim_domain))
                ...
                for i in range(len(batch_index)):
                    ...
                    label_domain[i] = self.y_train_domain[batch_index[i]]
   

                with self.lock:
                    self.data_buffer = data, label,label_domain
            sleep(0.0001)
    ...
    def iterate_batch(self):
        ...
        data, label,label_domain = self.data_buffer
        data = numpy.asarray(data, dtype=numpy.float32)
        label = numpy.asarray(label, dtype=numpy.int32)
        label_domain = numpy.asarray(label_domain, dtype=numpy.int32)
        with self.lock:
            self.data_buffer = None

        return data, label,label_domain
...
if __name__ == '__main__':
    ...
    y_train_domain = numpy.zeros((120, 10))
    ...
    for i in range(10):
        ...
        data, label,label_domain = model.iterate_batch()

        print('max: %f, min: %f' % (data.max(), data.min()))
        
        print (data.shape)
        print (label.shape)
        print (label_domain.shape)
